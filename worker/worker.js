...
function buildUserPrompt(input, gender) {
...
function isJsonRelated(err) {
  const m = err?.message || '';
  return m.includes('JSON') || m.includes('parse') || m.includes('position') || m.includes('token');
}
